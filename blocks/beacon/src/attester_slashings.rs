use common::{
    ...
    utils::{array_to_string, bytes_to_hex},
};
...
pub fn insert_attester_slashings(tables: &mut DatabaseChanges, clock: &Clock, attester_slashings: &[AttesterSlashing]) {
    for (index, attester_slashing) in attester_slashings.iter().enumerate() {
        ...
        // Attestation 1
        let attestation_1_attesting_indices = array_to_string(&attestation_1.attesting_indices);
        ...
        // Attestation 2
        let attestation_2_attesting_indices = array_to_string(&attestation_2.attesting_indices);
