...
pub fn insert_account_ram_delta(tables: &mut DatabaseChanges, clock: &Clock, action: &ActionTrace, transaction: &TransactionTrace, account_ram_delta: &AccountRamDelta) {
    // transaction
    ...
    let keys = account_ram_delta_keys(clock, &tx_hash, &action_index, account, delta);
    let row = tables
